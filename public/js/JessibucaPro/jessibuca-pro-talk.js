!(function (t) {
  ...
  class t {
    ...
    emit(t) {
      for (
        ...
        s[r - 1] = arguments[r];
      for (let t = 0; t < e.length; t += 1) e[t].fn.apply(e[t].ctx, s);
      return this;
    }
    off(t, i) {
      ...
      if (t) {
        ...
        if (s && i)
          for (let t = 0, e = s.length; t < e; t += 1)
            s[t].fn !== i && s[t].fn._ !== i && r.push(s[t]);
  ...
  const r = 'debug',
    o = 'warn',
    s = 'talkGetUserMediaSuccess',
    a = 'talkGetUserMediaFail',
          ...
          return new Promise(
            function (t, e) {
              var i = function () {
                ...
                s =
                  (this.on('change', i),
                  (r = r || o.documentElement)[n.requestFullscreen](a));
            ...
            function (t, e) {
              ...
              this.isFullscreen
                ? ((i = function () {
                  ...
                  this.on('change', i),
                  (s = o[n.exitFullscreen]()) instanceof Promise &&
                    s.then(i).catch(e))
                : t();
            }.bind(this),
          );
  ...
  try {
    if (
      ...
      var _ = new WebAssembly.Module(
        Uint8Array.of(0, 97, 115, 109, 1, 0, 0, 0),
      );
      if (_ instanceof WebAssembly.Module)
        new WebAssembly.Instance(_) instanceof WebAssembly.Instance;
    }
  } catch (t) {}
  class v {
    constructor(t) {
    ...
    linearInterpolation() {
      this.resampler = (t) => {
        ...
        for (
          ...
          for (
            a = s % 1, r = 1 - a, this.lastWeight = s % 1, h = 0;
            h < this.channels;
            ++h
          )
            l[n++] = this.lastOutput[h] * r + t[h] * a;
        for (--s, u -= c, o = Math.floor(s) * c; n < e && o < u; ) {
          for (a = s % 1, r = 1 - a, h = 0; h < this.channels; ++h)
            l[n++] = t[o + (0 < h ? h : 0)] * r + t[o + (c + h)] * a;
      ...
      this.resampler = (t) => {
        ...
        do {
          if (l) for (r = s, a = 0; a < p; ++a) i[a] = 0;
            ...
            for (r = this.lastWeight, a = 0; a < p; ++a)
              i[a] = this.lastOutput[a];
            l = !0;
          }
          for (; 0 < r && o < d; ) {
            if (((n = 1 + o - c), !(r >= n))) {
          ...
          if (0 !== r) {
            for (this.lastWeight = r, a = 0; a < p; ++a)
              this.lastOutput[a] = i[a];
            this.tailExists = !0;
            break;
          }
  ...
  const T = [255, 511, 1023, 2047, 4095, 8191, 16383, 32767];
  function y(e, i, s) {
  ...
  class S {
    constructor(a) {
      (this.log = function (t) {
        if (a._opt.debug && a._opt.debugLevel == r) {
          const r = a._opt.debugUuid ? `[${a._opt.debugUuid}]` : '';
          for (
            var e = arguments.length, i = new Array(1 < e ? e - 1 : 0), s = 1;
            s < e;
            s++
          )
            i[s - 1] = arguments[s];
          console.log(`JbPro${r}[✅✅✅][${t}]`, ...i);
        }
      }),
        (this.warn = function (t) {
          if (
            a._opt.debug &&
            (a._opt.debugLevel == r || a._opt.debugLevel == o)
          ) {
            const r = a._opt.debugUuid ? `[${a._opt.debugUuid}]` : '';
            for (
              var e = arguments.length, i = new Array(1 < e ? e - 1 : 0), s = 1;
        ...
        (this.error = function (t) {
          for (
            var e = a._opt.debugUuid ? `[${a._opt.debugUuid}]` : '',
              i = arguments.length,
              s = new Array(1 < i ? i - 1 : 0),
              r = 1;
            r < i;
  ...
  class U extends t {
    constructor(t) {
      ...
      (this._opt = Object.assign({}, t, e)),
        ...
        (this.events = new M(this)),
        this._initTalk(),
        this.player || (this.debug = new S(this)),
        (this._opt.encType !== p && this._opt.encType !== f) ||
    ...
    destroy() {
      this.clearWorkletUrlTimeout &&
        ...
        this.socket &&
          (this.socketStatus === d && this._sendClose(),
          this.socket.close(),
          (this.socket = null)),
    ...
    addRtpToBuffer(t) {
      var e = t.length + this.tempRtpBufferList.length,
        e = new Uint8Array(e);
      e.set(this.tempRtpBufferList, 0),
        e.set(t, this.tempRtpBufferList.length),
        (this.tempRtpBufferList = e);
    }
    downloadRtpFile() {
    ...
    _log(t) {
      for (
        ...
        i[s - 1] = arguments[s];
      (this.player ? this.player.debug : this.debug || console)[t](...i);
    }
    _getSequenceId() {
        ...
        (this.socket = new WebSocket(this.wsUrl)),
          ...
          i(this.socket, 'close', (t) => {
            (this.socketStatus = 'close'),
              this.warn(this.tag, 'websocket close -> reject', t),
              this.emit(l),
              e(t);
          }),
          i(this.socket, 'error', (t) => {
            (this.socketStatus = 'error'),
              this.error(this.tag, 'websocket error -> reject', t),
              this.emit(h, t),
              e(t);
          });
      });
    ...
    _initTalk() {
      this._initMethods(),
        this._opt.engine === e
          ? this._initWorklet()
          : 'script' === this._opt.engine && this._initScriptProcessor(),
        this.log(
          this.tag,
    ...
    _initScriptProcessor() {
      ...
      (this.recorder = t.apply(this.audioContext, [
        this.bufferSize,
    ...
    _initWorklet() {
      ...
      this.audioContext.audioWorklet &&
        this.audioContext.audioWorklet.addModule(t).then(() => {
          var t = new AudioWorkletNode(this.audioContext, 'talk-processor', {
            processorOptions: { bufferSize: this.bufferSize },
          });
          t.connect(this.gainNode),
            (t.port.onmessage = (t) => {
              'data' === t.data.eventType &&
                this._encodeAudioData(t.data.buffer);
            }),
            (this.workletRecorder = t);
        }),
        (this.clearWorkletUrlTimeout = setTimeout(() => {
          URL.revokeObjectURL(t), (this.clearWorkletUrlTimeout = null);
        }, 1e4));
    }
    _onaudioprocess(t) {
    ...
    _encodeAudioData(t) {
      if (0 === t[0] && 0 === t[1]) this.log(this.tag, 'empty audio data');
      else {
        const s = this.resampler.resample(t);
        let e = s;
        if (
            ...
            : 8 === this._opt.sampleBitsWidth &&
              (e = (function (t) {
                ...
                for (; e--; ) {
                  var s = Math.max(-1, Math.min(1, t[e]));
                  i[e] = parseInt(
                    255 / (65535 / (32768 + (s < 0 ? 32768 * s : 32767 * s))),
                    10,
                  );
                }
                return i;
          ...
          this._opt.encType === p
                  ...
                  Array.prototype.slice.call(t).forEach((t, e) => {
                    i[e] = (function (t) {
                      ...
                      return (
                        0 <= t
                          ? (e = 213)
                          : ((e = 85), (t = -t - 1) < 0 && (t = 32767)),
                        8 <= (i = y(t, T, 8))
                          ? 127 ^ e
                          : ((s = i << 4),
                            (s |= i < 2 ? (t >> 4) & 15 : (t >> (i + 3)) & 15) ^
                              e)
                      );
                    })(t);
                  }),
            ...
            : 'pcm' === this._opt.encType && (t = e);
          const s = new Uint8Array(t);
          for (let t = 0; t < s.length; t++) {
            var i = this.audioBufferList.length;
            (this.audioBufferList[+i] = s[t]),
    ...
    rtpPacket(t) {
      ...
      this._opt.encType === p
        ...
        : this._opt.encType === f
        ? (i = 0)
        : 'opus' === this._opt.encType && (i = 98),
        this.startTimestamp || (this.startTimestamp = b()),
        (r = b() - this.startTimestamp),
        (s = this._getSequenceId());
      let n = 0;
      if ('tcp' === this._opt.packetTcpSendType) {
        const t = o + 12;
        (e[n++] = 255 & (t >> 8)), (e[n++] = 255 & (t >> 0));
      }
      (e[n++] = 128),
        (e[n++] = 128 + i),
        (e[n++] = s / 256),
      ...
      for (let t = 0; t < l.length; t++) h[t] = l[t];
      return h;
    }
    opusPacket(t) {
      return t;
    ...
    _sendTalkMsg(t) {
      null === this.tempTimestamp && (this.tempTimestamp = b());
      var e = b(),
        i = e - this.tempTimestamp,
        s = this._parseAudioMsg(t);
      this.log(
        this.tag,
        `'send talk msg and diff is ${i} and byteLength is ${s.byteLength} and length is ${s.length}, and g711 length is ` +
          t.length,
      ),
        (!0 !== (i = this._opt.saveRtpToFile) && 'true' !== i) ||
          this._opt.packetType !== m ||
          this.addRtpToBuffer(s),
        s &&
          (this.socketStatusOpen
            ? this.socket.send(s.buffer)
            : this.emit('tallWebsocketClosedByError')),
        ...
        window.navigator.mediaDevices
          ...
          .then((t) => {
            this.log(this.tag, 'getUserMedia success'),
              ...
              (this.mediaStreamSource =
                this.audioContext.createMediaStreamSource(t)),
          ...
          .catch((t) => {
            this.error(this.tag, 'getUserMedia error', t.toString()),
              this.emit(a, t.toString());
          })
          .finally(() => {
    ...
    async _getUserMedia3() {
      ...
      try {
        ...
        console.log('getUserMedia() got stream:', t),
    ...
    _handleStreamInactive(t) {
      this.userMediaStream &&
        (this.warn(this.tag, 'stream oninactive', t), this.emit(u));
    }
    _startCheckGetUserMediaTimeout() {
      this._stopCheckGetUserMediaTimeout(),
        (this.checkGetUserMediaTimeout = setTimeout(() => {
          this.log(this.tag, 'check getUserMedia timeout'), this.emit(n);
    ...
    startTalk(i) {
      return new Promise((t, e) => {
        if (
          ...
          return e('not support getUserMedia');
        if (((this.wsUrl = i), this._opt.testMicrophone)) this._doTalk();
        else {
          if (!this.wsUrl) return e('wsUrl is null');
    ...
    setVolume(t) {
      ...
      (t = parseFloat(t).toFixed(2)),
        isNaN(t) ||
          ((e = t),
          (t = Math.max(Math.min(e, Math.max(0, 1)), Math.min(0, 1))),
  ...
  class R extends t {
      ...
      var t =
        ...
        t =
          (this.talk &&
            ...
            this.talk.destroy(),
            (this.talk = null)),
          Object.assign({}, w(this._opt), t));
      (this.talk = new U(null, t)),
            ...
            .then(() => {
              t(),
                ...
                this.talk.once(n, () => {
                  this.debug.warn(
                    this.LOG_TAG,
                    'talkGetUserMediaTimeout -> stopTalk',
                  ),
                    this.stopTalk()
                      .catch((t) => {
                        this.debug.warn(
    ...
    setTalkVolume(i) {
      return new Promise((t, e) => {
        this.debug.log(this.LOG_TAG, 'setTalkVolume', i),
          this.talk || e('talk is not init'),
          this.talk.setVolume(i / 100),
          t();
      });
    }
    downloadTempRtpFile() {
  ...
  (R.EVENTS = i), (window.JessibucaProTalk = R), (window.WebPlayerProTalk = R);
});
