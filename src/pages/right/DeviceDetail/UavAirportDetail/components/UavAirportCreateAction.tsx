...
const UavAirportCreateAction: FC<PropsType> = memo(() => {
  ...
  const createAction = async () => {
    ...
    try {
      ...
      const { data } = await getCameraByType(
        deviceDetail.childDevice?.at(0)?.properties.gimbalType || 'H20T',
      )
      const camera = data
...
UavAirportCreateAction.displayName = 'UavAirportCreateAction'

export default UavAirportCreateAction
