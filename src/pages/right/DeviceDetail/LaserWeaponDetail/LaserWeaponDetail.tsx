...
const LaserWeaponDetail: FC<PropsType> = memo(
  ({ data, headerTools, onClose }) => {
    const header = useMemo(
      () => (
        <div className="flex gap-2 items-center">
          <DeviceIconLaser className="device-detail-icon" />
          ...
          <div className="flex-1 overflow-y-auto">
            <AppViewSuspense>
              {tab === '详情' ? (
                ...
                <CameraDetailData deviceId={deviceId} deviceType={deviceType} />
              )}
            </AppViewSuspense>
          </div>
        </div>
...
LaserWeaponDetail.displayName = 'LaserWeaponDetail'

export default LaserWeaponDetail
