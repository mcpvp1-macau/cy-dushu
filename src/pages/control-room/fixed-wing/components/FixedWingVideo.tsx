  ...
  return (
    <div className="size-full relative bg-black">
      ...
      <aside className="absolute top-3 left-3 flex gap-2 items-center z-10">
        {(['tv', 'ir'] as const).map((mode) => (
          <button
            key={mode}
            className={clsx(
              'h-7 px-3 text-xs rounded-sm border border-solid cursor-pointer',
              sensorMode === mode
                ? 'text-sky-100 bg-sky-800/80 border-sky-500/60'
                : 'text-fore bg-ground-3/80 border-ground-3',
            )}
            onClick={() =>
              useFixedWingDemoStore.getState().updateSensorMode(mode)
            }
          >
            {mode === 'tv' ? '电视' : '红外'}
          </button>
      ...
      {/* 右上角激光状态 */}
      <aside className="absolute top-3 right-3 z-10">
        <span className="h-7 px-3 text-xs rounded-sm bg-ground-3/80 text-fore flex items-center gap-1.5">
          激光
          <span
            className={clsx(
