 ...
 *
 * @param {Scene_} scene The scene.
 * @param {Cartesian3_} position The position in world (WGS84 or alternative ellipsoid) coordinates.
 * @param {Cartesian2_} [result] An optional object to return the input position transformed to window coordinates.
 * @param {Camera_} camera
 * @param {number} width
 * @param {number} height
 * @param {Cartesian2} [result]
 * @returns {Cartesian2_|undefined} The modified result parameter or a new Cartesian2 instance if one was not provided.  This may be <code>undefined</code> if the input position is near the center of the ellipsoid.
 *
 * @example
...
SceneTransforms.worldWithEyeOffsetToWindowCoordinates = function (
  ...
  if (frameState.mode === SceneMode.SCENE2D) {
    ...
    if (
      ...
      windowCoordinates.x >= width
    ) {
      cameraCentered = true
    } else {
      if (windowCoordinates.x > width * 0.5) {
      ...
      if (result.x < 0.0 || result.x > width) {
 ...
 * const handler = new Cesium.ScreenSpaceEventHandler(scene.canvas);
 * handler.setInputAction(function(movement) {
 *     console.log((Cesium.SceneTransforms.worldToDrawingBufferCoordinates ??
...
SceneTransforms.wgs84ToWindowCoordinates = function (scene, position, result) {
  deprecationWarning(
    'SceneTransforms.wgs84ToWindowCoordinates',
    'SceneTransforms.wgs84ToWindowCoordinates has been deprecated. It will be removed in 1.21. Use SceneTransforms.worldToDrawingBufferCoordinates instead.',
  )
