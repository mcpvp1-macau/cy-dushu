...
/** 绘制多边形 */
const DrawPolygon: FC<PropsType> = memo(({ showDistance, onDrawEnd }) => {
  ...
  const getMidPoint = useMemoizedFn((p1: number[], p2: number[]) => [
    ...
    viewer?.scene?.globe?.getHeight(
      Cesium.Cartographic.fromDegrees((p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2),
    ) ?? 0,
  ])
  ...
  useEffect(() => {
    ...
    handler.setInputAction((e) => {
      ...
      setPath((prev) => [...prev, [geo[0], geo[1], geo[2]]])
    ...
    handler.setInputAction((e) => {
      ...
      // 地形上的点
      const geo = cartesian3ToDegrees(cartesian)
      setMousePoint([geo[0], geo[1], geo[2]])
  ...
  return (
    <>
      ...
      {mousePoint && <GroundPolygon positions={[...path, mousePoint]} />}
